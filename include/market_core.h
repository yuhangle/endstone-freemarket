...
public:
    ...
    //获取玩家资金
    int umoney_get_player_money(const std::string& player_name);
    //更改玩家资金
    [[nodiscard]] bool umoney_change_player_money(const std::string& player_name, int money) const;

    //读取玩家可提现物品
    vector<ItemStackData> UserItemRead(const string& uuid);
    ...
    //获取玩家资产
    int get_player_money(const endstone::Player& player);
    ...
    // 检查是否允许触发事件
    bool canTriggerEvent(const string& playername);
